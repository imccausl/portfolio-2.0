...
gulp.task("build:moveCNAME", () => {
  return gulp.src("dev/CNAME").pipe(gulp.dest("dist/"));
});

gulp.task("files", () => {
  return gulp.src("dev/files/**/*").pipe(gulp.dest("dist/files"));
});

gulp.task("moveBundle", () => {
  return gulp.src("dev/js/bundle.js").pipe(gulp.dest("dist/js"));
});

gulp.task("routes", () => {
  return gulp.src("dev/routes/**/*").pipe(gulp.dest("dist/routes"));
});
...
gulp.task("watch", () => {
  gulp.watch("dev/scss/*.scss", ["sass:watch"]);
...
gulp.task("build", callback => {
  runSequence(
    "clean:dist",
    ["webpack", "sass", "fonts", "images", "routes", "files", "setToIndex"],
    "useref:build",
    "removeAppHTML",
    "moveBundle",
    "build:moveCNAME",
